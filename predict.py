...
def main(model_class, model_weights_filename, tokenizer_filename, embeddings_filename, _config):
    ...
    f = open(os.environ['FPATH'], 'w')
    probabilities = get_prediction(model, tokenizer, premise, hypothesis)
    for con, neu, ent in probabilities:
        line = str(con) + '  ' + str(neu) + ' ' + str(ent)
        con, neu, ent = float(con), float(neu), float(ent)
        if con >= ent and con >= neu:
            f.write('Contradiction'+ '::' + line + '\n')
        #elif ent > neu and ent > con:
        #    f.write('Entailment'+ '::' + line + '\n')
        elif con <= ent and neu <= ent:
            f.write('Entailment'+ '::' + line + '\n')
        else:
            f.write('Neutral' + '::' + line + '\n')
